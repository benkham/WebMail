...
namespace WebMail.SendGrid
{
    public class SendGridSender : ISender
    {
        ...
        {
            var host = _configuration.GetSection("SENDGRID_HOST").Value;
            var port = Convert.ToInt32(_configuration.GetSection("SENDGRID_PORT").Value);
            var user = _configuration.GetSection("SENDGRID_USER").Value;
            // API key is the password for SendGrid
            var apiKey = _configuration.GetSection("SENDGRID_API_KEY").Value;
            var credentials = new NetworkCredential(user, apiKey);
